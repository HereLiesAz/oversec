...
abstract class AbstractTextEncryptionInfoFragment : Fragment() {
    private val TAG = "AbstractTextEncryptionInfoFragment"
    ...
    open fun setData(
        ...
        try {
            tvSize.text = requireActivity().getString(
                R.string.bytes_size,
            ...
            if (tdr.isOk) {
                ...
                try {
                    ...
                    if (innerData.hasTextAndPaddingV0()) {
                        ...
                        tvInnerPadding.text = requireActivity().getString(
                            R.string.bytes_size,
                ...
                var m = requireActivity().getString(Stuff.getErrorText(tdr.error!!))
        ...
        if (uix == null) {
            ...
            btAction.setOnClickListener {
                try {
                    val atdr = CryptoHandlerFacade.getInstance(requireActivity())
    ...
    open fun onOptionsItemSelected(activity: Activity, item: MenuItem): Boolean {
        val id = item.itemId
        if (id == R.id.action_share_encrypted) {
            ...
            AlertDialog.Builder(activity)
                .setTitle(R.string.confirmation_share_decrypted__title)
                .setMessage(R.string.confirmation_share_decrypted__content)
                .setPositiveButton(R.string.action_share) { _, _ ->
                    try {
                        ...
                        if (innerData.hasTextAndPaddingV0()) {
                            ...
                            Log.w(
                                TAG,
                                "Can't share inner data of type ${mTdr!!.decryptedDataAsInnerData.dataCase.name}"
                            )
                        }
                    } catch (e: InvalidProtocolBufferException) {
                        try {
                            ...
                            Log.w(TAG, "Can't share inner data!")
                        }
